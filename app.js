...
window.addEventListener("scroll", () => {
    let scrollPos = window.scrollY;
    story_img.style.transform = `translateY(${scrollPos * -0.2}px)`; // Adjust speed here
});
window.addEventListener("scroll", () => {
    let scrollPos = window.scrollY;
...
"assets/coffee-svgrepo-com.svg",
  ...
  const svgs = [];
  const positions = []; // Store positions to prevent overlap
  ...
  // Function to move SVGs randomly inside the container (without overlap)
  function moveSVGs() {
  ...
  // Move SVGs every 2 seconds
  setInterval(moveSVGs, 2000);
  
  moveSVGs(); // Initial movement
